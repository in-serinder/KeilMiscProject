...
// 每个通道的CE状态（1=充电，0=停充）
static uint8_t ce_state[4] = {1, 1, 1, 1};
...
void main(void) {
  ...
  while (1) {
    ...
    // 设置模式
    if (setting_mode) {
      uint8_t cur_th = KEY_GetThreshold();
      // 仅在值变化或首次进入时刷新（防抖并降低I2C开销）
      if (cur_th != cha_th) {
        cha_th = cur_th;
        SSD13XX_ClearPage(0);
        SSD13XX_ClearPage(1);
    ...
    cha_th = KEY_GetThreshold(); // 同步按键阈值到充电控制阈值
