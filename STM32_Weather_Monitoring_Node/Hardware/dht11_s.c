...
#define DHT(x) ((x) ? GPIO_SetBits(GPIOA, GPIO_Pin_2) : GPIO_ResetBits(GPIOA, GPIO_Pin_2))
...
uint8_t checkDHT11_S(void)
{
    ...
    // 等待DHT11拉低（响应信号）
    while (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_2) && retry < 100) {
        retry++;
        Delay_us(1);
    }
