#include "Delay.h"
#include "UART.h"
#include "pcf8574.h"
#include "stc89c52.h"

#ifndef __FAKELOAD_H__
#define __FAKELOAD_H__
// 主要是假负载的电阻控制定义
#define R10 PCF8574_P0   // P0 功耗耐受10w
#define R20_1 PCF8574_P1 // P1 功耗耐受20w
#define R20_2 PCF8574_P2 // P2 功耗耐受20w
#define R20_3 PCF8574_P3 // P3 功耗耐受20w
#define R20_4 PCF8574_P4 // P4 功耗耐受20w
#define R100 PCF8574_P5  // P5 功耗耐受5w
// #define R10KR PCF8574_P6 | PCF8574_P7
// #define RESISTANCE_LIST_SIZE 64
#define RESISTANCE_LIST_SIZE 13 // 安全阈值简化后

void FakeLoad_Init(void);
uint8_t FakeLoad_SetPower(float power, float voltage);
void FakeLoad_SetResistance(uint8_t resistance_index);
void FakeLoad_Reset(void);
void FakeLoad_Set(uint8_t port, uint8_t value);
float FakeLoad_getPower(uint8_t resistance_index, float voltage);
float FakeLoad_getResistance(uint8_t resistance_index);
void FakeLoadTest(void);

#endif